...
public class McpToolCallbackResolver {
    ...
    /**
     * 解析结果（T22）：本次真正挂上去的工具，以及**是否发生了降级**——调用方据此给用户一句可见提示。
     * <p>
     * {@code degraded=true} 的含义是"**MCP 是配置启用的，但这次没拿到工具**"（服务不可达、冷却期内、
     * 或其他会话探测在途）；Bean 不存在属配置选择（{@code AI_MCP_CLIENT_ENABLED=false} 等），
     * 是 {@code degraded=false}——那种情况不该给用户报"服务不可用"。
     * </p>
     */
    public record McpTools(ToolCallback[] callbacks, boolean degraded) {

        static McpTools ok(ToolCallback[] callbacks) {
            return new McpTools(callbacks, false);
        }

        static McpTools ofDegraded() {
            return new McpTools(NO_TOOLS, true);
        }
    }

    /**
     * 解析搜图 MCP 工具回调，失败一律降级（本方法不抛异常）。
     *
     * @param providerSupplier 取 MCP 工具回调 Bean（不存在时返回 null）；用 Supplier 而不是直接传 Bean，
     *                         是为了让"取 Bean"与"列工具"两步的失败都能落到同一个降级路径上
     */
    public McpTools resolve(Supplier<ToolCallbackProvider> providerSupplier) {
        if (isCoolingDown()) {
            log.info("搜图 MCP 工具处于降级冷却期，本次会话不挂载搜图工具（到期自动重试）");
            return McpTools.ofDegraded();
        }
        ...
        try {
            ...
            if (provider == null) {
                ...
                return McpTools.ok(NO_TOOLS);
            }
            ToolCallback[] callbacks = provider.getToolCallbacks();
            nextAttemptNanos.set(NO_COOLDOWN);
            return McpTools.ok(callbacks == null ? NO_TOOLS : callbacks);
        } catch (Exception e) {
