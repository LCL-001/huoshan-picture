...
- **前端怎么发也没定**（2026-09-16 补记）：仓里没有任何前端发布说明（`grep npm run build` 在 `docs/` 与 `deploy/` 里零命中）。注意 `frontend/package.json` 的 `build` = `run-p type-check build-only`，而 type-check **有 124 例存量错误基线**（AGENTS.md），这条命令很可能非 0 退出；`pure-build` 才是只做 vite build 的那条。上线前要确认实际用哪条、`dist/` 怎么传到服务器、静态目录在哪。
