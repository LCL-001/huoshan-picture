...
- 部署：**部署清单与 nginx 片段在 `deploy/`（入库）**——`deploy/prod-checklist.md`（AI 助手一期的 prod 配置、启动顺序、上线自检表，含两个会直接坑住人的陷阱）与 `deploy/nginx/ai-assistant-sse.conf`（SSE 的 location 片段）。历史习惯位置 `backend/docs/deploy/` 被 `backend/.gitignore` 的 `docs/` 规则排除、**不在版本控制内**，只当本机遗留看，别再往里加东西（2026-09-15 复核发现）。CI（.github/workflows/ci.yml）跑编译与无 MySQL/Redis 单测
