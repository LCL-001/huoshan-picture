...
8. **凭证组"只判存在性"的实际边界（review 实测，与 decisions 2026-09-14 那条的表述有出入）**：代理放行**任意非空白 satoken**——用真会话 Cookie + 假 token `review-bogus-token` 打代理，引擎照常运行、`listSpaces` 读到真实空间。也就是说"两把凭据不一致会被发现"只对空间维度读图成立（`checkSpaceViewPermission` 回 40102），`listSpaces` 之类的路径实际只由 Spring Session 授权。加固方案（代理用 `StpKit.SPACE` 现成 API 校验两把同属一人）属推翻已决口径，需用户拍板（decisions.md R4）。
