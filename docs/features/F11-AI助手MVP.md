...
> 2026-09-16 **T27 第一阶段**：首次公开部署暂时摘掉两个直接写工具，助手恢复为**查询 + 搜图候选 + 建议**；
> 写工具实现保留但不装配，完成确定性确认门前不会由助手直接改标签或按 URL 入库。实施证据见 `docs/plans/records/T27-first-release.md`。

## 一句话

登录后在 `/assistant` 页面与内嵌 AI 助手对话：助手只处理**火山图库、空间、图片、素材、标签、分类、整理、素材搜索及平台权限**相关事项，并按当前账号权限查询空间、图片与标签词表；管理员还可让它看图并给出标签/分类建议。**当前助手只提供查询、候选和建议，不会直接修改标签、上传图片或删除数据。**执行步骤以折叠条逐步显示，回答进气泡。

## 怎么用

1. 顶栏菜单或左侧浮动栏点「AI 助手」（入口在任何页面都在；移动端在汉堡抽屉里）。
2. 空状态给了四个示例问题，点一下即填入输入框：`列出我的空间`、`看看某个空间里有哪些图片`、`看看这些图片适合哪些标签`、`帮我搜索一些图片素材`；`Enter` 发送、`Shift+Enter` 换行。
...
4. **当前助手不直接改数据（T27 第一阶段）**：它可以查询空间/图片/标签、搜索候选素材；管理员会话可用 `visionTagger` 看图并给出标签/分类建议，但建议不会自动落库。`batchEditPictures` 与 `batchUploadByUrl` 的实现仍保留，确定性用户确认门完成前不装配给模型，因此任何“已修改 / 已入库 / 已删除”的说法都不成立。需要实际修改时，请先使用普通图库页面。
...
8. 助手不是通用聊天机器人。编程、旅游、百科、写作、翻译等**整条都与图库无关**的问题会统一回复「我只能协助处理火山图库中的空间、图片、标签、整理和素材搜索等事项。」；这类纯范围外请求不会调用 `listSpaces` 等图库工具。混合请求会处理其中合法的图库事项，并拒绝范围外部分。
...
浏览器 EventSource(credentials)  →  GET /api/ai/assistant/chat?message=&chatId=&ticket=
   ↓ AiAssistantController 四道门槛：① getLoginUser（Spring Session 登录态）
        ② 一组凭据齐备：Sa-Token（配置名对应的请求头或 HttpOnly Cookie；不接受 query）+ 会话 Cookie（配置名对应的原样值）
   ...
   ↓ AiAssistantProxyManager（专用线程）：HttpURLConnection GET 引擎
        ...
        头：X-Internal-Api-Key（配置）、Sa-Token 配置名对应的原样值、X-User-Role（T17：调用者角色，空值不发）、Accept: text/event-stream
        Cookie：会话 Cookie 配置名对应的浏览器原样值；禁止自动跟随 3xx
   ↓ 引擎：先做一次不注册任何工具的范围分类（只有严格 IN_SCOPE 才放行）
         ├─ 范围外 / 非协议结果 → 固定范围提示 + [DONE]，不进入正式模型与工具链
         └─ 范围内 → 图库助手会话类型 → 当前工具集（三个只读图库工具 + MCP searchImage；**外加 visionTagger——仅当配置了视觉模型且调用者是管理员**；两个直接写工具暂不装配；各工具带调用者凭据打图库 API，RBAC 由图库判）
...
12. **能力口径经历过两次调整，当前以 T27 第一阶段为准**：T14 曾在档 3 放开写工具后把页面从“只读”回填为“明确要求即可修改”；T26 又补了四条操作示例。2026-09-16 的 T27 安全 review 发现写操作只有提示词确认、没有确定性执行门，因此首次公开部署重新暂时摘掉 `batchEditPictures` / `batchUploadByUrl`，页面和本功能文档已同步为“查询 + 搜图候选 + 建议，不直接写入”。历史写能力不是当前部署承诺；恢复写入须完成 T27.4～T27.7。前端错误态 UI 与工具摘要增强仍按 `F12` 已知限制记录。
