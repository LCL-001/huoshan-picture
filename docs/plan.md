...
- [ ] T10 图库后端 AI 代理模块：新增代理端点（转发对话请求到 ai/agent headless SSE 端点）——服务间 API key 认证（配置化，密钥不入库）、**当前用户凭据组透传（satoken 头 + `SESSION` 会话 Cookie，B 口径 2026-09-14 拍板；缺任一把引擎侧入口即拒 40000）**、用户标识（huoshan:<userId>）传递、SSE 流转发（SB 2.7 实现选型实施时定：SseEmitter/OkHttp EventSource）
