...
- [x] T25（2026-09-16 用户要求修复）**约束助手自然语言回答范围**：实现口径见 `docs/spec.md` 关键三行；当前问题与实施证据见 `docs/plans/records/T25.md`。
...
- [ ] T27（2026-09-16 后端安全 review；同日用户放行第一阶段前 4 项）**AI 助手后端安全加固**：收口代理凭据重定向泄露、query satoken、SSE 早取消竞态、日志/异常原文外泄、错误体无界读取、凭据名硬编码，以及写工具缺少确定性确认门。
  - **第一阶段前 4 项已完成（T27 整体仍未完成）**：T27.1 凭据与重定向加固；暂时摘掉 `batchEditPictures` / `batchUploadByUrl`；Prompt/工具日志与未知异常脱敏；页面与功能文档同步为只读/建议能力。实施与红绿证据见 `docs/plans/records/T27-first-release.md`。
  - 后续：T27.2 的其余取消竞态/错误体有界读取可继续修；完整写确认协议仍按 T27.4 先立字据，放行后再做 T27.5～T27.8。
  - 边界：不重复处理 F11 已关闭的多行 SSE、缺 `[DONE]`、线程池拒绝空 500、300 秒超时、GET message 日志、频控/长度限制等已知限制；不新增进程/依赖/数据表。
  - 详细任务、文件范围、负向控制、提交拆分与停止条件：见 `docs/plans/2026-09-16-T27-AI助手后端安全加固实施计划.md`。
...
| # | 事实 | 判定 |
|---|---|---|
| ~~A1~~ | **历史线上曾跑修复前的后端版本**：2026-09-15 匿名实测 `POST /api/space/list/page/vo` 回 `code:0` + 7 个空间完整元数据（含 `userId`/`userAccount`/`userName`/头像/空间规模）——正是 T3.2 要封的"匿名可枚举空间与属主"，而该集成测试断言为"匿名 40100"。即 T3.1/T3.2/T3.3 那批安全修复**已完成、已独立 review，但未部署**。信息暴露性质为**只读的信息泄露**（账号名与空间元数据；邮箱字段为 `null`），无写入、无账号接管 | **2026-09-17 已随新后端部署自然收口**；此前用户接受暴露的历史判定保留，不再作为现存风险。 |
...
| ~~D3~~ | ~~nginx SSE 配置入库并合入生产~~ | SSE 必须禁用缓冲并把读取超时放宽到应用侧 300s 以上 | **已完成（2026-09-17）**：宝塔 nginx 已合并 SSE / WebSocket / 普通 API 片段，助手线上可用；长对话逐帧与 360s 边界尚无文本证据。配置见 `deploy/nginx/ai-assistant-sse.conf`，上线增量见最新 handoff |
| ~~D4~~ | ~~引擎与 MCP 的 prod 配置、建库与守护方式~~ | Java 21、空库建表、模型/Redis/内部密钥、回环监听与进程托管缺一都会卡住首次上线 | **已完成（2026-09-17）**：生产库 `huoshan_ai_agent` 四表已导入，三个 Java 进程由宝塔托管，环境变量已生效；真机发现 MySQL 用户必须用宝塔实际账户 `huoshan_ai_agent`。完整清单与未复测项见 `deploy/prod-checklist.md`，过程见最新 handoff |
| ~~D5~~ | ~~部署后勾 spec 的 F11~~ | 用户 2026-09-15 拍板"等一期部署上线后再勾" | **已完成（2026-09-17）**：首次生产部署完成，`docs/spec.md` 的 F11 已勾选 |
...
### 数据卫生（**活跃的探针数据已于 2026-09-15 清理完毕**）

- **已清（2026-09-15，用户要求）**：30 个探针账号、31 个测试/无主空间、22 张测试图、9 个 `T5新词*` 测试词、19 行悬空成员、11 个死会话。活跃用户 38→**8**、活跃空间 37→**6**、活跃词条 26→**17**。**用户自己在用的 8 个账号一行未动**（`1234` 有 144 图、`admin` 有 10 图）。过程、两次 dry-run 救回的错、可复跑脚本见 `docs/plans/records/probe-data-purge-2026-09-15.md` 与 `backend/sql/cleanup/2026-09-15-purge-probe-data.sql`。
- **仍未清（另开任务才做）**：**逻辑删除的历史残留**——`user` 377 行 / `space` 206 行 / `picture` 362 行（`isDelete=1`）。对应用不可见，但占库、且**唯一索引仍被占用**（同名词条/账号不可再注册；这条对词表已在 `decisions.md` 记过）。清理要跨表物理删除，属独立任务。
- **附带代价**：被删的 22 张图是**物理删除**，其 COS 对象未回收（SQL 直删绕过了应用的异步清理），都是测试图。
- **规则沉淀**：本次两次 dry-run 各救回一个错——① MySQL 的 `LIKE` **不支持 `[_]` 字符类**，写 `'sec[_]%'` 会匹配 0 条、静默漏删（精确前缀一律用 `left(col,N)='...'`）；② **不能照账号名猜是不是探针**（`1234`/`admin` 是带 144/10 张图的在用账号，删前必须看内容足迹）。
