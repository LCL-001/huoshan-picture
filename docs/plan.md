...
- [ ] **MVP 档位与推进顺序（2026-09-14 用户拍板"先做出来能用，再慢慢优化"）**——一期范围不变，只调顺序与颗粒度：
  ...
  - **档 2 前置项（2026-09-14 档 1 冒烟发现，须先决策再开工）**：实测 `/space/list/page/vo` 的登录态只认 **Spring Session**（`SpaceController.java:142` → `UserServiceImpl.getLoginUser` 读 `USER_LOGIN_STATE`），**不吃 sa-token**——新签发的有效 satoken 单独调用仍是 `40100`，仅带 `SESSION` cookie 才成功；而 `/picture/list/page/vo`（公共图库，无鉴权）与空间维度图片读接口（走 sa-token）不受影响。故图库助手的第一个工具 `listSpaces` 在"只透传 satoken"的现行设计下拿不到数据，T10 必须二选一收口：**A（推荐）** 给空间接口加 sa-token 回退（对齐 decisions 2026-09-13「空间接口全部依赖 Sa-Token 登录态」，且不把第二把凭据带进引擎）／**B** 代理额外转发 SESSION、工具透传两把凭据（改动小，但把 Spring Session 引入引擎，而它正是"改密后不失效"的残余风险面）。口径见 docs/decisions.md 待决策行。
  - **档 2 用户可见 MVP（本轮终点）**：T10（backend 代理：服务间 API key + satoken 透传 + SSE 转发）+ T11 最小形态（助手入口 + 对话面板 + SSE 步骤折叠条）。验收：登录后在界面里对话、折叠条逐步可见；收口时做一次里程碑独立 review（规则 13）+ 一篇合并的功能讲解（规则 9）。
  ...
  - **MVP 阶段放宽**：规则 13 独立 review 与规则 9 讲解文档降为里程碑（档 2 收口）一次；重集成测试留到 T12；上轮 review 的 P3（反射断言精简、`CONTROL_TIMEOUT` 余量、一把尺拆分、T6 的 ③⑥）继续挂着。**不放宽**：密钥不入库 / token 只在请求生命周期不落库、一期不注册删除类工具、工具调用走用户真实 satoken 由图库服务端判 RBAC、门禁红绿灯、AGENTS/docs 改动用 `docs:` 前缀单独提交。
