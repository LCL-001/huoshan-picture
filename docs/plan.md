...
| D4 | 引擎与 MCP 的 prod 配置与环境变量 | 两份 prod yaml 均不入库（设计如此），需要一份可执行的部署清单。**注意 D1 已把"只监听回环"写进入库的 `application.yaml`，故该修复对 prod 自动生效**（prod 那份未覆盖 `server.address`） | **已产出（2026-09-15）**：`deploy/prod-checklist.md`。**并列了三个陷阱**：① 三个 jar 里都烤进了本机的 `application-local.yaml`，而后端默认 profile 就是 `local`（拿本机构建的 jar 直接起会连本机库、用本机密钥）；② 引擎的 `thinking: disabled` 不在入库配置里，prod 必须显式给（不给则 MiMo 进思考模式、`temperature` 失效）；③ **引擎必须给一个真实可用的 `AI_DASHSCOPE_API_KEY`**（2026-09-16 实测：key 为空 → `DashScope API key must be set`；把 chat 关成 `none` → `FlowWindowBasedChatMemory` 找不到 `ChatModel` Bean，两种都起不来）——而且**它不是摆设**，长对话的摘要压缩走的就是它（`summarize()` 里 `chatModel.call()`）。另记两件会卡住首次部署的事：**仓库还没 push**（若在服务器 clone/pull 会拿到两个月前的代码）、**引擎从未以 prod 形态从空库起过** |
