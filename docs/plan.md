...
- [ ] T27（2026-09-16 后端安全 review；同日用户放行第一阶段前 4 项）**AI 助手后端安全加固**：收口代理凭据重定向泄露、query satoken、SSE 早取消竞态、日志/异常原文外泄、错误体无界读取、凭据名硬编码，以及写工具缺少确定性确认门。
  - **本轮已放行范围**：① T27.1 凭据与重定向加固；② 暂时从助手装配中摘掉 `batchEditPictures` / `batchUploadByUrl`（类与测试保留，普通图库编辑/上传不变）；③ Prompt/工具参数/工具结果日志及工具未知异常脱敏；④ 页面文案与示例同步成只读/建议能力。`n  - 后续：T27.2 的其余取消竞态/错误体有界读取可继续修；完整写确认协议仍按 T27.4 先立字据，放行后再做 T27.5~T27.8。
