...
- **T17 浏览器侧两种角色验收**：引擎侧装配已真机验收（`%TEMP%\t23\acceptance.txt`，**不可复跑，证据已过期**），UI 表现未看。
- **T22 `notice` 行渲染**：只过了 type-check/eslint 与帧级契约，未在浏览器里看过那一行。
- 顺带可看：普通用户问打标时模型的话术（提示词未按角色补话术）。
