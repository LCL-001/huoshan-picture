...
- [ ] T27（2026-09-16 后端安全 review，用户要求生成修复计划）**AI 助手后端安全加固**：收口代理凭据重定向泄露、query satoken、SSE 早取消竞态、日志/异常原文外泄、错误体无界读取、凭据名硬编码，以及写工具缺少确定性确认门。
  - 实施顺序：T27.1~T27.3 可直接按红绿修复；T27.4 先补“写操作确定性确认”字据并等用户确认，放行后再做 T27.5~T27.8。
