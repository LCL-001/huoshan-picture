...
- [ ] T6.1（P1，**T7/T8 开工前必做**）流式路径超时保护：`OpenAiChatModels` 装配 `OpenAiApi` 时补 `webClientBuilder(...)`，给 `WebClient` 一个带读取超时（连接超时一并设）的 connector；并补单测钉住"阻塞与流式两条路径的超时均已接到 HTTP 客户端上"
  - 现象：`OpenAiApi.Builder` 无参构造默认 `webClientBuilder = WebClient.builder()`（字节码），而 `OpenAiChatModels#build` 只调 `restClientBuilder(...)`
  - 证据（T6 review 实测，本机挂死端点探针）：非流式 `chatCompletionEntity` 走 restClient——`timeout=3s` 时 3046 ms 抛 `java.net.SocketTimeoutException: Read timed out`；流式 `chatCompletionStream` 走 webClient——同一端点、同一 3s 配置，25 s 后仍 pending（无超时、无报错）。反射实测 `JdkClientHttpConnector.readTimeout=null`、底层 `java.net.http.HttpClient.connectTimeout=Optional.empty`（本项目 classpath 无 reactor-netty/jetty/HC5，`WebClient.builder()` 的 connector 探测按序落到 JdkClientHttpConnector）
  - 影响：T8 把对话循环收敛为"仅 SSE 流式"后，上游一次卡死（TCP 不断开、无字节）即永久挂住 SSE 连接与请求线程；且不产生异常，Spring AI 重试模板（仅 retryOn `TransientAiException`）无从介入
  - 验收：单测（两条路径超时断言，含流式 connector 的 readTimeout 非空）+ 挂死端点冒烟（阻塞与流式均在超时内失败）
  - 文件范围：ai/agent 的 config/OpenAiChatModels.java + 对应测试
