...
| D4 | 引擎与 MCP 的 prod 配置与环境变量 | 两份 prod yaml 均不入库（设计如此），需要一份可执行的部署清单。**注意 D1 已把"只监听回环"写进入库的 `application.yaml`，故该修复对 prod 自动生效**（prod 那份未覆盖 `server.address`） | **已产出（2026-09-15）**：`deploy/prod-checklist.md`。**并列了两个陷阱**：① 三个 jar 里都烤进了本机的 `application-local.yaml`，而后端默认 profile 就是 `local`（拿本机构建的 jar 直接起会连本机库、用本机密钥）；② 引擎的 `thinking: disabled` 不在入库配置里，prod 必须显式给（不给则 MiMo 进思考模式、`temperature` 失效）。原第三个陷阱（**引擎必须给一个真实可用的 `AI_DASHSCOPE_API_KEY`**，2026-09-16 实测 key 为空与关掉 `model.chat` 两种都起不来）**已于同日收口**：引擎侧不再有任何 DashScope 用法（对话主脑 / 看图打标 / 摘要 / 标题全走主脑 MiMo），入库配置改为 `model.chat: none` + `model.embedding: none` + `dashscope.enabled: false`，实测无 key 可启动。口径与证据见 `docs/decisions/2026-09-16-dashscope-decoupling.md`。另记两件会卡住首次部署的事：**仓库还没 push**（若在服务器 clone/pull 会拿到两个月前的代码）、**引擎从未以 prod 形态从空库起过**。**2026-09-16 更新**：引擎侧 Flyway 已整体移除（口径见 `docs/decisions/2026-09-16-flyway-removed.md`），空库不再自建表，首次部署必须先执行 `deploy/sql/engine-schema.sql`（`deploy/prod-checklist.md` 第 5 节已按此改写）。**2026-09-16 上线前命名收口**：生产与默认引擎库统一为 `huoshan_ai_agent`，见 `docs/decisions/2026-09-16-engine-database-name.md` |
